...
export const getCachedSatoshis = (amount: string): string | undefined => {
  return conversionCache[amount + InterchainedUnit.LOCAL_CURRENCY];
};

export const setCachedSatoshis = (amount: string, sats: string): void => {
  conversionCache[amount + InterchainedUnit.LOCAL_CURRENCY] = sats;
};
...
type AmountInputProps = Omit<TextInputProps, 'onChangeText' | 'value'> & {
  ...
  amount?: string;
  /**
   * The current unit of the amount (ITC, SATS, LOCAL_CURRENCY)
   */
   ...
   * Returns a InterchainedUnit value
   */
  onAmountUnitChange: (unit: InterchainedUnit) => void;
};
...
export const AmountInput: React.FC<AmountInputProps> = props => {
  ...
  const secondaryDisplayCurrency = useMemo(() => {
    if (amount === InterchainedUnit.MAX) {
      return '';
    }
    switch (unit) {
      case InterchainedUnit.ITC: {
        const sat = new BigNumber(amount).multipliedBy(100000000).toNumber();
        return formatBalanceWithoutSuffix(sat, InterchainedUnit.LOCAL_CURRENCY, false);
      }
      case InterchainedUnit.SATS:
        return formatBalanceWithoutSuffix(Number(amount), InterchainedUnit.LOCAL_CURRENCY, false);
      case InterchainedUnit.LOCAL_CURRENCY: {
        let res: string = '';
        if (conversionCache[amount + InterchainedUnit.LOCAL_CURRENCY]) {
          // cache hit! we reuse old value that supposedly doesn't have rounding errors
          const sats = conversionCache[amount + InterchainedUnit.LOCAL_CURRENCY];
          res = satoshiToITC(Number(sats));
        } else {
          res = fiatToITC(Number(amount));
        }
        res = removeTrailingZeros(res);
        return `${res} ${loc.units[InterchainedUnit.ITC]}`;
      }
    }
  }, [amount, unit]);
  ...
  const changeAmountUnit = useCallback(() => {
    ...
    let newUnit;
    // cycle through units ITC -> SAT -> LOCAL_CURRENCY -> ITC
    if (previousUnit === InterchainedUnit.ITC) {
      newUnit = InterchainedUnit.SATS;
    } else if (previousUnit === InterchainedUnit.SATS) {
      newUnit = InterchainedUnit.LOCAL_CURRENCY;
    } else if (previousUnit === InterchainedUnit.LOCAL_CURRENCY) {
      newUnit = InterchainedUnit.ITC;
    } else {
      newUnit = InterchainedUnit.ITC;
      previousUnit = InterchainedUnit.SATS;
    }

    /**
     * here we must recalculate old amont value (which was denominated in `previousUnit`) to new denomination `newUnit`
     * and fill this value in input box, so user can switch between, for example, 0.001 ITC <=> 100000 sats
     */
    ...
    switch (previousUnit) {
      ...
      case InterchainedUnit.SATS:
        ...
        break;
      case InterchainedUnit.LOCAL_CURRENCY:
        sats = new BigNumber(fiatToITC(+amount)).multipliedBy(100000000).toString();
        break;
    }
    if (previousUnit === InterchainedUnit.LOCAL_CURRENCY && conversionCache[amount + previousUnit]) {
    ...
    if (newUnit === InterchainedUnit.LOCAL_CURRENCY && previousUnit === InterchainedUnit.SATS) {
  ...
  const handleChangeText = useCallback(
    (text: string) => {
      text = text.trim();
      if (unit !== InterchainedUnit.LOCAL_CURRENCY) {
        text = text.replace(',', '.');
        ...
        text = unit === InterchainedUnit.ITC ? text.replace(/[^0-9.]/g, '') : text.replace(/[^0-9]/g, '');
      } else {
  ...
  const stylesHook = StyleSheet.create({
    center: { padding: amount === InterchainedUnit.MAX ? 0 : 15 },
      ...
      <View style={styles.root}>
        ...
        <View style={styles.flex}>
          <View style={styles.container}>
            {unit === InterchainedUnit.LOCAL_CURRENCY && amount !== InterchainedUnit.MAX && (
              <Text style={[styles.localCurrency, stylesHook.localCurrency]}>{getCurrencySymbol() + ' '}</Text>
            )}
            {amount !== InterchainedUnit.MAX ? (
              <TextInput
                onSelectionChange={handleSelectionChange}
                testID="InterchainedAmountInput"
                ...
                value={amount === InterchainedUnit.MAX ? loc.units.MAX : parseFloat(amount) >= 0 ? String(amount) : undefined}
              ...
              <Pressable onPress={resetAmount}>
                <Text style={[styles.input, stylesHook.input]}>{InterchainedUnit.MAX}</Text>
              </Pressable>
            )}
            {unit !== InterchainedUnit.LOCAL_CURRENCY && amount !== InterchainedUnit.MAX && (
        ...
        {!disabled && amount !== InterchainedUnit.MAX && (
          <TouchableOpacity
