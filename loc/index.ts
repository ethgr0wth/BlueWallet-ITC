 ...
 * @param withFormatting {boolean} Works only with `InterchainedUnit.SATS`, makes spaces wetween groups of 000
 * @returns {string}
 */
export function formatBalance(balance: number, toUnit: string, withFormatting = false): string {
  if (toUnit === undefined) {
    return balance + ' ' + loc.units[InterchainedUnit.ITC];
  }
  if (toUnit === InterchainedUnit.ITC) {
    const value = new BigNumber(balance).dividedBy(100000000).toFixed(8);
    return removeTrailingZeros(+value) + ' ' + loc.units[InterchainedUnit.ITC];
  } else if (toUnit === InterchainedUnit.SATS) {
    return (withFormatting ? new Intl.NumberFormat().format(balance).toString() : String(balance)) + ' ' + loc.units[InterchainedUnit.SATS];
  } else {
 ...
 *
 * @param balance {number} Satoshis
 * @param toUnit {string} Value from models/bitcoinUnits.js, for example `InterchainedUnit.SATS`
 * @param withFormatting {boolean} Works only with `InterchainedUnit.SATS`, makes spaces wetween groups of 000
 * @returns {string}
 */
