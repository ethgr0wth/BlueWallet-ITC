...
describe('Localization', () => {
  ...
  it('formatBalancePlain() && formatBalancePlain()', () => {
    _setExchangeRate('ITC_RUB', 660180.143);
    _setPreferredFiatCurrency(FiatUnit.RUB);
    let newInputValue = formatBalanceWithoutSuffix(152, InterchainedUnit.LOCAL_CURRENCY, false);
    assert.ok(newInputValue === 'RUB 1.00' || newInputValue === '1,00 ₽', 'Unexpected: ' + newInputValue);
    newInputValue = formatBalancePlain(152, InterchainedUnit.LOCAL_CURRENCY, false);
    assert.strictEqual(newInputValue, '1');

    newInputValue = formatBalanceWithoutSuffix(1515, InterchainedUnit.LOCAL_CURRENCY, false);
    assert.ok(newInputValue === 'RUB 10.00' || newInputValue === '10,00 ₽', 'Unexpected: ' + newInputValue);
    newInputValue = formatBalancePlain(1515, InterchainedUnit.LOCAL_CURRENCY, false);
    assert.strictEqual(newInputValue, '10');

    newInputValue = formatBalanceWithoutSuffix(16793829, InterchainedUnit.LOCAL_CURRENCY, false);
    assert.ok(newInputValue === 'RUB 110,869.52' || newInputValue === '110 869,52 ₽', 'Unexpected: ' + newInputValue);
    newInputValue = formatBalancePlain(16793829, InterchainedUnit.LOCAL_CURRENCY, false);
    assert.strictEqual(newInputValue, '110869.52');

    newInputValue = formatBalancePlain(76, InterchainedUnit.LOCAL_CURRENCY, false);
    ...
    newInputValue = formatBalancePlain(16000000, InterchainedUnit.LOCAL_CURRENCY, false);
    assert.strictEqual(newInputValue, '1600');
  });

  it.each([
    [123000000, InterchainedUnit.SATS, false, '123000000', false],
    [123000000, InterchainedUnit.SATS, true, '123,000,000', false],
    [123456000, InterchainedUnit.ITC, true, '1.23456', false],
    ['123456000', InterchainedUnit.ITC, true, '1.23456', false], // can handle strings
    [100000000, InterchainedUnit.ITC, true, '1', false],
    [10000000, InterchainedUnit.ITC, true, '0.1', false],
    [1, InterchainedUnit.ITC, true, '0.00000001', false],
    [10000000, InterchainedUnit.LOCAL_CURRENCY, true, '...', true], // means unknown since we did not receive exchange rate
  ])(
    'can formatBalanceWithoutSuffix',
    async (balance, toUnit, withFormatting, expectedResult, shouldResetRate) => {
      _setExchangeRate('ITC_USD', 1);
      _setPreferredFiatCurrency(FiatUnit.USD);
      if (shouldResetRate) {
        _setExchangeRate('ITC_USD', false);
  ...
  it.each([
    [123000000, InterchainedUnit.SATS, false, '123000000 sats'],
    [123000000, InterchainedUnit.ITC, false, '1.23 ITC'],
    [123000000, InterchainedUnit.LOCAL_CURRENCY, false, '$1.23'],
  ])(
    'can formatBalance',
