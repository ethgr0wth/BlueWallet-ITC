...
describe('PayjoinTransaction', () => {
  it('throws if smth is wrong with pj transaction', async () => {
    ...
    const payjoinClient = new PayjoinClient({
      paymentScript: Buffer.from(uint8ArrayToHex(interchained.address.toOutputScript('bc1qyvdzueznsh0rsyfqzdtj9ce7nlx4rlg2v93lcl')), 'hex'),
  ...
  it('works', async () => {
    ...
    const w = new HDSegwitBech32Wallet();
    w.setSecret(process.env.MNEMONICS_COLDCARD);
    // interchained:bc1qy0ydthpa35m37pvwl5tu76j0srcmcwtmaur3aw?amount=0.0001&pj=https://btc.donate.kukks.org/ITC/pj
    ...
    // created cransaction:
    const psbtOrigin = interchained.Psbt.fromBase64(
    ...
    const txOrigin = interchained.Transaction.fromHex(
    ...
    const wallet = new PayjoinTransaction(
      ...
      async txhex => {
        broadcastWasCalled++;
        const tx2broadcast = interchained.Transaction.fromHex(txhex);
    ...
    const payjoinRequesterMock = {
      requestPayjoin: async function () {
        ...
        return interchained.Psbt.fromBase64(
    ...
    const payjoinClient = new PayjoinClient({
      paymentScript: Buffer.from(uint8ArrayToHex(interchained.address.toOutputScript('bc1qy0ydthpa35m37pvwl5tu76j0srcmcwtmaur3aw')), 'hex'),
      // @ts-ignore idk too lazy to figure out, seems to work
      wallet,
      // @ts-ignore idk too lazy to figure out, seems to work
      payjoinRequester: payjoinRequesterMock,
