...
class SpecifyInterfaceController: WKInterfaceController {
  ...
  struct SpecificQRCodeContent {
    ...
    var bitcoinUnit: NumericKeypadInterfaceController.NumericKeypadType = .ITC
  }
  var specifiedQRContent: SpecificQRCodeContent = SpecificQRCodeContent(amount: nil, description: nil, amountStringArray: ["0"], bitcoinUnit: .ITC)
  var wallet: Wallet?
  ...
  override func awake(withContext context: Any?) {
    ...
    self.specifiedQRContent.bitcoinUnit = (wallet.type == .lightningCustodianWallet) ? .SATS : .ITC
